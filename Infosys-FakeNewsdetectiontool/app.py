...
class GeminiChat(db.Model):
    ...
    model_used = db.Column(db.String(50), default='gemini-1.5-pro-latest')
...
def save_analysis_background(user_id, content, result, url=None, title=None, is_quick=True):
    ...
    def save_task():
        try:
            ...
            key_findings_json = json.dumps(result.get('key_findings', [])[:2])  # Save only 2 findings

            with app.app_context():
                analysis = Analysis(
                    user_id=user_id,
                    title=title or f"Analysis {datetime.now(timezone.utc).strftime('%H:%M')}",
                    content=content[:1000],  # Save only first 1000 chars to DB
...
# Change this route name from gemini_status to debug_gemini_status
@app.route('/debug/gemini-status')
def debug_gemini_status():  # Changed from gemini_status
...
# Then add your /api/gemini/status route with a different function name
@app.route('/api/gemini/status')
def api_gemini_status():  # Different function name
...
def gemini_status():
    ...
    return jsonify({
        'success': True,
        'available': gemini_assistant.available,
        'model': _GEMINI_MODEL if gemini_assistant.available else 'none',
        'api_key_configured': bool(_GEMINI_API_KEY and _GEMINI_API_KEY != 'your-gemini-api-key-here'),
...
def analysis_detail(analysis_id):
    ...
    if analysis.is_quick_analysis:
        ...
        fact_checks = json.loads(analysis.fact_checks) if analysis.fact_checks else []
...
def init_database():
    with app.app_context():
        try:
            ...
            try:
                # Try to create individual tables if bulk create fails
                ...
                """))
                db.session.commit()
                app.logger.info("GeminiChat table created individually")
        ...
        if not User.query.filter_by(email=admin_email).first():
            admin = User(email=admin_email, name='System Administrator',
